...
class PlayState extends MusicBeatState
{
	...
	private function separateMobileArrows():Void
	{
		isMobileHUD = true;
		setOnScripts('isMobileHUD?', true);

		for (note in unspawnNotes)
			if (!note.mustPress)
				note.visible = false;

		var oppY:Float = ClientPrefs.data.downScroll ? 40 : 590;
		var plrY:Float = ClientPrefs.data.downScroll ? 550 : 70;

		for (i in 0...4)
		{
			var oppStrum:StrumNote = opponentStrums.members[i];
			var plrStrum:StrumNote = playerStrums.members[i];

			oppStrum.y = oppY;
			plrStrum.y = plrY;

			oppStrum.x = 10 + (i * 65);
			plrStrum.x = mobileNoteXPositions[i];

			oppStrum.scale.x /= 1.75;
			oppStrum.scale.y /= 1.75;
			oppStrum.updateHitbox();
		}

		mobileHitbox = new mobile.controls.Hitbox();
		mobileHitbox.setLayout(mobileNoteXPositions, plrY - 20, 150, 150);
	...
	private function keysCheck():Void
	{
		...
		#if mobile
		if (isMobileHUD && mobileHitbox != null)
		{
			for (dir in ["left", "down", "up", "right"])
			{
				holdArray.push(mobileHitbox.pressed(dir));
				pressArray.push(mobileHitbox.justPressed(dir));
				releaseArray.push(mobileHitbox.justReleased(dir));
			}
		}
		else
		#end
		...
		#if mobile
		var usingAltInput:Bool = controls.controllerMode || isMobileHUD;
		#else
		var usingAltInput:Bool = controls.controllerMode;
		#end

		if(usingAltInput && pressArray.contains(true))
		...
		if((usingAltInput || strumsBlocked.contains(true)) && releaseArray.contains(true))
